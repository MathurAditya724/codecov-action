...
export async function writeJobSummary(
  ...
  // Main header
  summary.addHeading("Codecov Action Results 📊", 1);
...
async function addTestResultsSection(
  ...
  summary.addHeading("Test Results 🧪", 2);
  ...
  if (results.comparison) {
    summary.addHeading("📊 Comparison with Base Branch", 3);
    ...
    if (results.comparison.testsBroken.length > 0) {
      ...
      summary.addDetails(
        `🔴 Tests Broken (${results.comparison.testsBroken.length})`,
        brokenTestsList
      );
    }
    ...
    if (results.comparison.testsFixed.length > 0) {
      ...
      summary.addDetails(
        `🟢 Tests Fixed (${results.comparison.testsFixed.length})`,
        fixedTestsList
      );
    }
    ...
    if (results.comparison.testsAdded.length > 0) {
      ...
      summary.addDetails(
        `➕ New Tests (${results.comparison.testsAdded.length})`,
        addedTestsList
      );
    }
    ...
    if (results.comparison.testsRemoved.length > 0) {
      ...
      summary.addDetails(
        `➖ Removed Tests (${results.comparison.testsRemoved.length})`,
        removedTestsList
      );
    }
  }
